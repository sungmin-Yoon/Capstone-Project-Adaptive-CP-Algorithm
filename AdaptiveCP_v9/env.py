...
class AdaptiveCPEnv:
    ...
    ── v8 변경 사항 (전부 유지) ──────────────────────────────────────────────
    [v8 수정 1] CP 하한 강화: sigma_ds → sigma_ds × 1.1
      - ISI 발생 원천 차단 (cp < sigma_ds 구간 진입 불가)
      - 10% 안전 마진으로 채널 추정 오차 흡수

    [v8 수정 2] is_nlos 플래그를 state 6번째 차원으로 추가
      - state = [σ_DS, SNR, Δθ, M, CP, is_nlos]  (6차원)
      - 에이전트가 NLOS 진입을 1스텝 후 즉시 인식 → 선제적 CP 상향 정책 학습

    [v8 수정 3] 보상 클리핑 [-20, 20]
      - 이상치 보상으로 인한 학습 불안정 억제

    [v8 준수 수정] delta_theta_deg 버그 수정
      - LOS: theta_b = theta_c → Δθ = 0 (논문 물리 모델)
      - _set_los_channel(): 0.0 고정
      - step(): abs(theta_b_deg - theta_c_deg) 로 업데이트

    [v8 준수 수정] theo_cp 논문 정의 복원: TCP = ρ·σ_DS = 3·σ_DS
      - CP_SAFETY_MARGIN(1.1)은 ISI 차단 clamp 전용 (theo_cp와 별개)

    ── v9 변경 사항 ──────────────────────────────────────────────────────────
    [v9] env.py 자체 변경 없음
      - 모든 수정은 test.py에 집중 (NLOS 타이밍 충돌 해소)
      - step() 내 nlos_event 처리 로직은 train.py / test.py 양쪽에서 동일하게 호출됨
    """
    ...
    # CP 하한 안전 마진 (ISI 차단용 clamp — theo_cp와 별개)
    CP_SAFETY_MARGIN = 1.1

    # State 정규화 기준값 — 6차원 (is_nlos 포함)
    STATE_NORM = np.array([
        200.0,   # σ_DS 최대 기준 (ns)
        30.0,    # SNR 최대 (dB)
        45.0,    # Δθ 최대 (°)
    ...
    def _set_los_channel(self):
        ...
        # [v8 준수 수정] LOS: θb = θc → Δθ = 0 (논문 물리 모델)
    ...
    # =========================================================================
    # step
    # [v5] NLOS 타이밍: nlos_event=True 시 행동 전 채널 먼저 변경
    # [v7] 보상 기준: fixed_se
    # [v8] CP 하한 σ×1.1, theo_cp=σ×ρ(논문 준수), 보상 클리핑
    # [v9] test.py가 nlos_event=is_nlos로 직접 호출 → 이 함수 자체는 변경 없음
    # =========================================================================
    def step(self, action: int, nlos_event: bool = False):

        # NLOS 급변 시 행동 전 채널 먼저 업데이트
        if nlos_event:
            self._set_nlos_channel()
        ...
        # theo_cp: 논문 정의 TCP = ρ·σ_DS (v8 준수 수정 유지)
        ...
        if not nlos_event:
            ...
            # [v8 준수 수정] LOS: Δθ = abs(θb - θc) = 0 (theta_b = theta_c 불변)
            self.delta_theta_deg = abs(self.theta_b_deg - self.theta_c_deg)
