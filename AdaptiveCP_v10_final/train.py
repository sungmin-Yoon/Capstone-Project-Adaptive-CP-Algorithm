...
# ── 안테나 배열 선택 ──────────────────────────────────────────────────────
# [v10] None = 랜덤 (M ∈ {16, 64, 256}), 16 / 64 / 256 = 해당 배열로 고정
FIXED_M = None
...
def train():
    ...
    m_label = f"M = {FIXED_M} (고정)" if FIXED_M is not None else "Random (M ∈ {16, 64, 256})"
    print(f"[Train] Using device: {device}")
    print(f"[Train] 안테나 배열: {m_label}")
    print(f"[Train] state_dim={env.state_dim}, action_dim={env.action_dim}")
    print(f"[Train] Episodes={EPISODES}, Steps/ep={STEPS_PER_EP}, Memory={MEMORY_SIZE}")
