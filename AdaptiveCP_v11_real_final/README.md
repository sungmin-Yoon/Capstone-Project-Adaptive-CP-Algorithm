...
### 7.1 평균 Spectral Efficiency (bps/Hz)

| M | No CP | Theoretical | Fixed CP | **Adaptive (DQN)** | Adaptive vs Fixed |
|---|---|---|---|---|---|
| 16 | 1.771 | 0.580 | 0.417 | **1.041** | **+149.6%** |
| 64 | 1.444 | 0.337 | 0.340 | **0.685** | **+101.6%** |
| 256 | 1.787 | 0.912 | 0.429 | **1.150** | **+168.1%** |

### 7.2 주요 지표 상세 (M = 64 기준)

| 지표 | No CP | Theoretical | Fixed CP | Adaptive (DQN) | Δ vs Fixed |
|---|---|---|---|---|---|
| σ_DS mean (ns) | — | 29.62 | 29.62 | 29.62 | — |
| SNR mean (dB) | — | 7.95 | 7.95 | 7.95 | — |
| CP mean (ns) | 0 | 88.86 | 75.00 | 36.00 | −38.99 ns |
| CP / σ_DS mean | 0 | 3.00 | 2.53 | 1.28 | −1.255 |
| SE mean (bps/Hz) | 1.444 | 0.337 | 0.340 | 0.685 | +101.6% |
| SINR mean (dB) | −0.74 | 4.30 | 3.99 | 1.66 | −2.34 dB |
| BER mean | 9.73e-02 | 1.17e-02 | 1.70e-02 | 4.40e-02 | ×2.6 worse |
| Power Eff. mean (%) | 100.0 | 12.03 | 11.76 | 24.12 | +12.36 pp |
| ISI Violations | 30/30 | 0 | 0 | 0/30 | — |

### 7.3 핵심 관찰

**① No CP SE > Fixed CP SE — 논문 핵심 포인트 검증**  
모든 M값, 모든 스텝에서 No CP SE가 Fixed CP SE를 상회하였다. Fixed CP(75 ns)는 실제 σ_DS(~30 ns) 대비 2.5배 과도하여, CP 오버헤드 손실(오버헤드 계수 ≈ 0.118)이 No CP의 ISI 패널티(×0.5)보다 크기 때문이다. 이는 논문이 Adaptive CP 필요성을 주장하는 근거를 수치로 확인한 것이다.

**② Adaptive CP의 트레이드오프 — SE ↑, SINR/BER은 절충**  
Adaptive DQN은 CP를 평균 36 ns(Fixed 75 ns 대비 −52%)로 운용하여 SE를 +101.6% 향상시키고 전력 효율도 +12.4 pp 개선하였다. 반면 CP가 짧아진 만큼 ISI 에너지 분율이 증가하여 SINR이 −2.3 dB 낮아지고 BER은 약 2.6배 높아진다. 이 트레이드오프는 SE 최대화를 목표로 설계된 보상 함수의 의도된 결과이다.

**③ ISI 위반 0건 (Adaptive)**  
CP 하한 클램프(σ_DS × 1.1)가 NLOS 급변 구간에서도 ISI를 완전히 차단하였다.

**④ M별 Adaptive SE 차이**  
M이 클수록(256) 빔이 좁아져 포착되는 멀티패스 성분이 줄고 σ_DS가 감소한다. 이에 따라 Adaptive CP도 더 짧게 유지되어 SE 향상폭이 커진다(M=256: +168.1%).
...
| σ_DS > T_s | No CP시 심각한 ISI **(본 시뮬레이션 해당: σ_DS ≈ 30 ns > T_s = 10 ns)** |

이 시뮬레이션에서는 σ_DS 평균 ≈ 30 ns, T_s = 1/100 MHz = 10 ns로 σ_DS > T_s이므로, No CP시 실제로도 유의미한 ISI가 발생하는 상황이다. BER 차트(10번)에서 No CP의 BER이 ~10^-1 수준으로 Fixed CP(~10^-2)보다 약 10배 높게 나오는 것이 이를 확인시켜 준다.

결론: 30/30은 코드 버그가 아니라, 이 시뮬레이션 파라미터 조건(σ_DS > T_s) 하에서 물리적으로도 타당한 결과이다.

### 8.2 SINR 역전 현상 (Fixed > Adaptive)

Fixed CP(75 ns)의 SINR이 Adaptive CP(~36 ns)보다 높게 나오는 것은 정상적인 결과이다.  
Fixed CP가 σ_DS의 약 2.5배 이상으로 유지되어 ISI가 거의 없는 반면, Adaptive CP는 의도적으로 CP를 줄여 SE를 높이는 대신 일부 ISI를 허용하기 때문이다.  
