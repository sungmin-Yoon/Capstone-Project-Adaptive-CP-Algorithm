...
def test():
    ...
    print("=" * 70)
    print("  Adaptive CP Algorithm — Test Phase (v5)")
    ...
    # ── 테스트 루프 ──────────────────────────────────────────────────────
    # [v5] NLOS 타이밍 수정:
    #   is_nlos가 True인 스텝에서 step() 호출 전에
    #   env의 채널 상태를 먼저 NLOS로 변경하고 state를 갱신.
    #   → 에이전트가 변화된 DS를 해당 스텝에서 즉시 관측하고 행동 결정.
    ...
    for step in range(1, NUM_STEPS + 1):
        ...
        # [v5] NLOS 시작 스텝에서 채널 상태를 행동 결정 전에 먼저 갱신
        if is_nlos:
            env._set_nlos_channel()
            env.sigma_ds = max(env._calculate_ds(), 1e-12)
            state        = env._make_state()   # 에이전트가 관측할 state 갱신

        # 에이전트 행동 결정 (갱신된 state 기반)
        with torch.no_grad():
            q_vals = model(torch.FloatTensor(state).unsqueeze(0).to(device))
        action = q_vals.argmax().item()

        # step 호출 (nlos_event=False: 채널 변경은 위에서 이미 완료)
    ...
    fig.suptitle(
        "Adaptive CP Algorithm — Performance Dashboard (v5)\n"
