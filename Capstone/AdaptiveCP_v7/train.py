...
def train():
    ...
    # ── [v7 수정 3] LR 스케줄러: StepLR → CosineAnnealingLR ──────────────
    # 기존: StepLR(step_size=500, gamma=0.1) → 500 에피소드 후 LR 10배 급감
    #       후반 500 에피소드에서 학습이 사실상 정체됨
    # 변경: CosineAnnealingLR → 코사인 곡선으로 완만하게 감소
    #       전체 에피소드에 걸쳐 LR이 0.001 → 0.0001로 부드럽게 변화
    #       후반에도 충분한 학습률을 유지하여 정책 개선 가능
