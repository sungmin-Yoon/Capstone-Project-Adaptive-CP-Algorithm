...
def test():
    ...
    print("=" * 70)
    print("  Adaptive CP Algorithm — Test Phase (v8)")
    print(f"  NLOS 급변 구간: Step {NLOS_START} ~ {NLOS_END}")
    print(f"  [v8] CP 하한: σ_DS × 1.1  |  Theoretical CP = σ_DS × 1.1")
    print("=" * 70)
    ...
    fig.suptitle(
        "Adaptive CP Algorithm — Performance Dashboard (v8)\n"
        f"(NLOS 급변 구간: Step {NLOS_START}~{NLOS_END}, 음영 표시 | "
        "Theoretical = σ_DS×1.1)",
    ...
    ax.plot(steps, theo_cp_list,  marker="s", linestyle="--", color="gray",
            linewidth=1.5, markersize=4, label="Theoretical CP (1.1·σ_DS)")
    ...
    ax.plot(steps, theo_se_list,  marker="s", linestyle="--", color="gray",
            linewidth=1.5, markersize=4, label="Theoretical SE (σ×1.1)")
    ...
    ax.axhline(1.1,  color="orange", linestyle="--", linewidth=1.0,
               alpha=0.7, label="v8 하한 (1.1×σ)")
    ax.axhline(2.0,  color="blue",  linestyle="--", linewidth=0.8,
               alpha=0.5, label="Sweet Spot 상한 (2.0×)")
