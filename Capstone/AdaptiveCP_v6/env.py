...
class AdaptiveCPEnv:
    ...
    ── v6 변경 사항 ──────────────────────────────────────────────────────────
    [원칙 1] 논문 수식 및 3GPP 파라미터 절대 준수
      - 3GPP μ, σ 값 유지, 클리핑 상한만 500ns → 100ns 조정
      - 클리핑 근거: 논문 참조[3][4] 28GHz 도심 실측 DS 범위(NLOS 50~200ns)

    [원칙 2] 보상 함수 재설계 (학습 이슈 해결)
      - 기존: new_se * 10 + 고정 페널티
      - 변경: (adaptive_se / theo_se) * 10 + 동적 낭비 페널티
      - 효과: CP가 이론값 대비 과도하게 클수록 페널티가 선형 증가
              → NLOS 종료 후 CP 과도 증가 문제 해결

    [원칙 3] NLOS 환경 정의 명확화
      NLOS 물리적 메커니즘:
        도심 산란체 증가 → 다중 경로 수 증가 → 평균 경로 지연(τ̂) 증가
        → Pc(τ) PDP가 더 긴 지연까지 분포 → σ_DS 증가
      구현: tau_hat을 3GPP NLOS 분포에서 샘플링
            빔포밍 파라미터(θb, θc, Δθ)도 NLOS 물리적 특성 반영
            (빔 방향이 주요 AoA에서 벗어남 → DS 추가 증가)
    """
    ...
    # [v6] 클리핑 상한: 500ns → 100ns
    # 근거: 논문 참조[3][4] 28GHz 도심 실측 DS 범위 기반
    TAU_HAT_MAX = 100e-9
    ...
    def _sample_tau_hat_3gpp(self, is_nlos: bool) -> float:
        ...
        [v6] 클리핑 상한을 100ns로 조정 (μ, σ 파라미터는 유지)
        """
    ...
    def _get_step_size(self) -> float:
        ...
        if ratio < 0.8:
            ...
            return max(self.sigma_ds * 0.05, 1e-9)
    ...
    def step(self, action: int, nlos_event: bool = False):
        ...
        # ── [v6] 보상 함수 재설계 ────────────────────────────────────────
        # 이론적 최적 SE 대비 현재 SE 비율 기반 보상
        # → CP가 이론값(σ_DS × ρ)에 가까울수록 높은 보상
        # → CP가 과도하게 클수록 동적 페널티 선형 증가
        theo_cp = self.sigma_ds * self.rho
        theo_se = self._calculate_se(theo_cp)

        if theo_se > 1e-10:
            se_ratio = new_se / theo_se
        else:
            se_ratio = 0.0

        reward = se_ratio * 10.0
        ...
        if self.current_cp < self.sigma_ds:
            ...
            # [v6] 동적 낭비 페널티: CP가 이론값 초과할수록 선형 증가
            # CP/theo_cp 가 2배면 -5, 3배면 -10, 5배면 -20
