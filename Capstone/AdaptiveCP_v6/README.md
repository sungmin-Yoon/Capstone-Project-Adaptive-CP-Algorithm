...
## 하이퍼파라미터 (train.py, v6 기준)

| 파라미터 | 값 | 설명 |
|---|---|---|
| LR | 0.001 | 학습률 (500 에피소드 후 0.1배 감소 — v7에서 수정 예정) |
| GAMMA | 0.99 | 할인율 |
...
**Changes**
- **env.py**: tau_hat 클리핑 상한 500ns → 100ns (3GPP μ/σ 파라미터 유지)
- **env.py**: 보상 함수 재설계: `(new_se / theo_se) × 10` 기반 + 동적 낭비 페널티  
  Sweet Spot(1.0~1.5×σ) 보너스 +3, ISI 페널티 -15, 과도 CP 선형 페널티 `-overrun×5`

**Issues — 코드 수준의 3가지 근본 문제**

#### 문제 1 (가장 심각): `_get_step_size()` 비대칭 — NLOS 이후 CP 회복 불가

```python
# env.py _get_step_size()
else:  # ratio > 1.5
    return max(self.sigma_ds * 0.05, 1e-9)   ← 약 0.5ns/step
```

NLOS 종료 후 `current_cp ≈ 250ns`, `sigma_ds ≈ 10ns` → `ratio = 25`.  
step_size = `10ns × 0.05 = 0.5ns`. 250ns → 30ns까지 **440스텝** 필요.  
테스트는 30스텝이므로 에이전트가 NLOS 이후 영구적으로 과도한 CP를 유지하며 SE 손실.  
차트 ⑦에서 CP/σ_DS 비율이 NLOS 이후 12 이상으로 폭등하는 것이 이 원인이다.

**수정 방향**: `ratio > 1.5` 구간을 세분화하여 ratio가 클수록 큰 step_size 부여.
```python
elif ratio < 5.0:
...
#### 문제 2: 보상 기준점 오류 — `theo_se`가 잘못된 목표를 제시

```python
# env.py step()
theo_cp = self.sigma_ds * self.rho         # = sigma_ds × 3.0
theo_se = self._calculate_se(theo_cp)
reward  = (new_se / theo_se) * 10.0
```

SE는 CP가 짧을수록 높아지므로, Sweet Spot(1~1.5×σ)에서의 SE는 theo_cp(3×σ) 기준 SE보다  
항상 높다 (`se_ratio > 1.0`). 반대로 NLOS에서 theo_cp가 200ns로 커지면 theo_se가 낮아져  
se_ratio가 과장되어 보상 스케일이 환경마다 다르게 변한다. 에이전트가 일관된 가치 함수를 학습하기 어렵다.

**수정 방향**: 보상 기준을 Fixed CP SE로 변경하여 "고정 CP 대비 개선량"으로 학습.
```python
fixed_se = self._calculate_se(self.fixed_cp)
reward   = (new_se - fixed_se) * 20.0
```

#### 문제 3: LR Scheduler 과도한 감쇠로 후반 학습 정체
...
1,000 에피소드 중 500 에피소드 이후 LR이 0.001 → 0.0001로 급감.  
차트 ①에서 후반부에도 보상 분산이 크고 이동평균이 수렴하지 않는 것이 이 원인으로 의심.  
아직 최적 정책 미수렴 상태에서 LR이 너무 낮아 탈출 불가.

**수정 방향**: CosineAnnealingLR으로 교체하여 완만한 감쇠 적용.
```python
scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=EPISODES, eta_min=1e-4)
```
...
| 버전 | Adaptive SE (전체) | NLOS SE | NLOS 보상 최저 | DS 변화 |
|---|---|---|---|---|
| v1 | — | — | -∞ | 없음 |
| v2 | — | — | -20 | 없음 (포화) |
...
| v6 | 0.263 ← 최저 | 0.229 | — | 6배 (100ns 클리핑) |
...
**v5 대비 v6 퇴보 원인 정리**  
v6의 보상 함수는 `se_ratio` 기반으로 환경마다 보상 스케일이 달라져 학습 일관성이 오히려 낮아졌다.  
tau_hat 클리핑(100ns)으로 DS 변동폭은 줄었으나, step_size 비대칭 문제가 여전히 존재하여  
NLOS 이후 CP 회복이 불가능한 상태가 지속되었다.

---

## v7 개선 방침

v6 분석을 통해 도출된 개선 방침. 수정 대상 파일과 위치를 명시한다.

| 파일 | 수정 위치 | 내용 |
|---|---|---|
| `env.py` | `_get_step_size()` | ratio 구간 세분화: `< 5.0 → σ×0.3`, `≥ 5.0 → σ×0.5` 추가 |
| `env.py` | `step()` 보상 계산 | 보상 기준: `theo_se` → `fixed_se` 기반 개선량으로 변경 |
| `train.py` | `scheduler` | `StepLR` → `CosineAnnealingLR(T_max=EPISODES, eta_min=1e-4)` |

**목표**: 논문/3GPP 준수를 유지하면서 Adaptive SE가 Fixed CP(0.330)를 전 구간에서 상회.
